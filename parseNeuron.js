// NEURONLIST LOGIC START
(()=>{

let CURRENT_IDX = null;

document.getElementById('neuronlist-back').addEventListener('click', () =>
{
    if (document.body.className !== 'state-neuronlist')
        return;
    document.body.className = 'state-neuronparse';
});

const decklistButton = document.getElementById('neuronlist-decklist');
...
const setupSingleList = ((which, idx, cards, countCheck) =>
{
    ...
    document.getElementById('neuronlist-count-'+which).innerText = (''+total);
    
    const container = document.getElementById('neuronlist-container-'+which);
    ...
    const checkElm = document.getElementById('neuronlist-check-'+which);
...
window.SetupNeuronList = (({main, extra, side}) =>
{
    CURRENT_IDX = null;
    let idx = {};
    ...
    CURRENT_IDX = idx;
    document.body.className = 'state-neuronlist';
    (async ()=>
    {
        const banlistCheck = document.getElementById('neuronlist-check-banlist');
    ...
    (async ()=>
    {
        const deckName = 'Neuron Export';
        const passcodeMap = await GetPasscodesFor(Object.keys(idx).map((k) => (+k)));
        const promises = {}
        for (const which of ['main','extra','side'])
        {
            const thisPromises = [];
            for (const {cardId, count} of document.getElementById('neuronlist-container-'+which).children)
...
nextButton.addEventListener('click', () =>
{
    ...
    StartLoading();
    window.SetupNeuronList(remapCurrentData());
});
